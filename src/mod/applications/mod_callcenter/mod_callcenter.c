	...
	if (!strcasecmp(key, "status")) {
		if (cc_agent_str2status(value) != CC_AGENT_STATUS_UNKNOWN) {
			...
			if (cc_agent_str2status(value) == CC_AGENT_STATUS_AVAILABLE) {
				sql = switch_mprintf("UPDATE agents SET status = '%q', last_status_change = '%" SWITCH_TIME_T_FMT "', talk_time = 0, calls_answered = 0, no_answer_count = 0, rejected_count = 0, busy_count = 0, not_registered_count = 0"
		...
		sql = switch_mprintf("UPDATE agents SET not_registered_delay_time = '%ld', system = 'single_box' WHERE name = '%q'", atol(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);

		result = CC_STATUS_SUCCESS;
	} else if (!strcasecmp(key, "max_rejected")) {
		sql = switch_mprintf("UPDATE agents SET max_rejected = '%d', system = 'single_box' WHERE name = '%q'", atoi(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE agents SET max_busy = '%d', system = 'single_box' WHERE name = '%q'", atoi(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE agents SET max_not_registered = '%d', system = 'single_box' WHERE name = '%q'", atoi(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
...
static switch_status_t load_config(void)
{
	...
	switch_cache_db_test_reactive(dbh, "select count(max_rejected) from agents", NULL, "alter table agents add max_rejected integer not null default 0;"
...
static void cc_agent_status_update(const char *agent_name, const char *agent_system, const char *member_uuid, const char *member_session_uuid, cc_agent_status_t newstatus, int count, int max, char *column_count, char *event_cc_action, char *event_cc_header_count, char *event_cc_header_status)
{
	switch_event_t *event = NULL;
	char *sql = NULL;
	...
	sql = switch_mprintf("UPDATE agents SET %q = %q + 1 WHERE name = '%q' AND system = '%q';", column_count, column_count, agent_name, agent_system);
	cc_execute_sql(NULL, sql, NULL);
	switch_safe_free(sql);
...
static void *SWITCH_THREAD_FUNC outbound_agent_thread_run(switch_thread_t *thread, void *obj)
{
	...
	if (status == SWITCH_STATUS_SUCCESS) {
		...
		if (bridged) {
			...
			sql = switch_mprintf("UPDATE agents SET uuid = '%q', last_bridge_start = '%" SWITCH_TIME_T_FMT "', calls_answered = calls_answered + 1, no_answer_count = 0, rejected_count = 0, busy_count = 0, not_registered_count = 0"
		...
		switch (cause) {
			...
			case SWITCH_CAUSE_USER_BUSY:
				...
				cc_agent_status_update(h->agent_name, h->agent_system, h->member_uuid, h->member_session_uuid, h->agent_busy_status, h->busy_count, 
										h->max_busy, "busy_count", "agent-max-busy", "CC-Agent-Busy-Count", "CC-Agent-Busy-Status");
				break;
			...
			case SWITCH_CAUSE_CALL_REJECTED:
				...
				cc_agent_status_update(h->agent_name, h->agent_system, h->member_uuid, h->member_session_uuid, h->agent_rejected_status, h->rejected_count, 
										h->max_rejected, "rejected_count", "agent-max-rejected", "CC-Agent-Rejected-Count", "CC-Agent-Rejected-Status");
				break;
			/* Protection againts super fast loop due to unregistrer */
			case SWITCH_CAUSE_USER_NOT_REGISTERED:
				delay_next_agent_call = 5;
				cc_agent_status_update(h->agent_name, h->agent_system, h->member_uuid, h->member_session_uuid, h->agent_not_registered_status, h->not_registered_count, 
										h->max_not_registered, "not_registered_count", "agent-max-not-registered", "CC-Agent-Not-Registered-Count", "CC-Agent-Not-Registered-Status");
				break;
			...
			default:
				...
				cc_agent_status_update(h->agent_name, h->agent_system, h->member_uuid, h->member_session_uuid, h->agent_no_answer_status, h->no_answer_count, 
										h->max_no_answer, "no_answer_count", "agent-max-no-answer", "CC-Agent-No-Answer-Count", "CC-Agent-No-Answer-Status");
				break;
		}
		...
		if (action && !strcasecmp(action, "load")) {
			...
			if (argc-initial_argc < 1) {
				...
				stream->write_function(stream, "%s",
				                       ...
				                       "max_wait_time_with_no_agent_time_reached|record_template|calls_answered|"\
				                       "calls_abandoned|ring_progressively_delay|skip_agents_with_external_calls|agent_no_answer_status|agent_rejected_status|agent_busy_status|agent_not_registered_status\n");
				switch_mutex_lock(globals.mutex);
