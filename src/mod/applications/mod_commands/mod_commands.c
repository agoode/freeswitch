...
SWITCH_STANDARD_API(originate_function)
{
	...
	char *mycmd = NULL, *myaleg = NULL, *argv[10] = { 0 }, *pipe_names[2] = { 0 };
	int i = 0, x, argc = 0;
	...
	if (argv[7]) {
		...
		/* Only comma delimited originate strings (forked calls) are supported in this mode */
		myaleg = strdup(aleg);
		switch_assert(myaleg);
		argc = switch_separate_string(myaleg, '|', pipe_names, (sizeof(pipe_names) / sizeof(pipe_names[0])));
		if (argc > 1 || strstr(aleg, SWITCH_ENT_ORIGINATE_DELIM)) {
			stream->write_function(stream, "-ERR | and %s delimiters are not supported in this mode\n", SWITCH_ENT_ORIGINATE_DELIM);
			goto done;
		}
