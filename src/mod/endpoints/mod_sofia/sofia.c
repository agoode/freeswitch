						...
						if ((a_session = switch_core_session_locate(br_a))) {
							...
							if(sofia_test_pflag(profile, PFLAG_FIRE_TRANFER_EVENTS)) {
								...
								if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_TRANSFEROR) == SWITCH_STATUS_SUCCESS) {
									...
									switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "att_xfer_original_call_id", br_b);
									switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "att_xfer_destination_call_id", br_a);
									switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "att_xfer_destination_peer_uuid", switch_core_session_get_uuid(a_session));
									switch_event_fire(&event);
								}
