...
#define MAX_QUEUE_LEN SWITCH_CORE_QUEUE_LEN /*100000*/
...
static void print_event_stats(listener_t *l, switch_log_level_t level) {
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(l->session), level, 
			...
			switch_queue_size(l->event_queue), MAX_QUEUE_LEN, l->event_stats[SWITCH_EVENT_ALL]);

	for (i = 0; i < SWITCH_EVENT_ALL; i++) {
		if ((l->event_stats[i]) != 0) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(l->session), level, "Event name: [%s] Count: [%lu]", switch_event_name(i), l->event_stats[i]); 
		}
	}
}
			...
			if (switch_event_dup(&clone, event) == SWITCH_STATUS_SUCCESS) {
				...
				if (qstatus == SWITCH_STATUS_SUCCESS) {
					if (l->lost_events) {
						...
						l->lost_events = 0;
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(l->session), SWITCH_LOG_CRIT, "Lost [%d] events! Event Queue size: [%u/%u] Total events / session: [%lu]\n", le, switch_queue_size(l->event_queue), MAX_QUEUE_LEN, l->event_stats[SWITCH_EVENT_ALL]);
					}
					...
					if (++l->lost_events > MAX_MISSED) {
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "Killing listener because of too many lost events! Lost [%d] Queue size[%u/%u] Total events / session: [%lu]\n", l->lost_events, qsize, MAX_QUEUE_LEN, l->event_stats[SWITCH_EVENT_ALL]);
