...
static switch_xml_t fetch_config(void)
{
...
static switch_status_t load_config(void)
{
	...
	switch_xml_t cfg, settings, param;
	char *next, *cur, *configText;
	uint32_t count = 0;
	uint8_t custom = 0;
	...
	/* Get the configuration XML, store it's hash, will test on reloadxml for changes */
	cfg = fetch_config();
	if (!cfg) return SWITCH_STATUS_TERM;
	configText = switch_xml_toxml(cfg, SWITCH_FALSE);
	switch_md5_string(module_state.configuration_md5, configText, (int) strlen(configText));
...
static switch_status_t initialize_sockets(void)
{
	int i = 0;
	...
	/* Separate memory pool for sockets, because it's so much easier to clean up at reload */
...
static void event_handler(switch_event_t *event)
{
	...
	if (event->event_id == SWITCH_EVENT_RELOADXML) {
		switch_xml_t cfg;
		char *configText;
		char tempMD5[SWITCH_MD5_DIGEST_STRING_SIZE] = { 0 };

		/* Has the configuration changed? */
		cfg = fetch_config();
		if (cfg) {
			configText = switch_xml_toxml(fetch_config(), SWITCH_FALSE);
			switch_md5_string(tempMD5, configText, (int) strlen(configText));
			switch_safe_free(configText);
		}

		/* If the config has changed, reload the configuration */
		if (cfg && (strcmp(module_state.configuration_md5, tempMD5) != 0)) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Configuration changed, reloading\n");
			switch_mutex_lock(module_state.mutex);

			/* Clean up the runtime struct */
			cleanup();

			/* Re-initialize the module */
			if (initialize_sockets() != SWITCH_STATUS_SUCCESS) {
			...
			switch_mutex_unlock(module_state.mutex);
		}
...
SWITCH_MODULE_LOAD_FUNCTION(mod_event_multicast_load)
{
	...
	memset(&globals, 0, sizeof(globals));
	switch_mutex_init(&module_state.mutex, SWITCH_MUTEX_NESTED, pool);
...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_event_multicast_shutdown)
{
	...
	switch_mutex_destroy(module_state.mutex);
...
SWITCH_MODULE_RUNTIME_FUNCTION(mod_event_multicast_runtime)
{
	...
	switch_assert(buf);
	module_state.running = 1;

	while (module_state.running == 1) {
		int rxdata = 0;
		...
		if (globals.has_udp == 1) {
			...
			if (module_state.running == 0 || (!SWITCH_STATUS_IS_BREAK(status) && status != SWITCH_STATUS_SUCCESS)) {
				break;
			}
